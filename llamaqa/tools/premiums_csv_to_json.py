...
def csv_to_json(csv_path, json_file_path):
    ...
    for _, row in df.iterrows():
        ...
        if age not in structured_data:
            structured_data[age] = {
                "medishield_premium": float(row['MediShield Life Premiums (Fully payable by Medisave)']),
                "additional_withdrawal_limit": float(row['Additional Withdrawal Limits (AWLs)']),
                "companies": {}
            }
        ...
        for col in df.columns[3:]:  # Skip 'Age', 'MediShield Life Premiums', and 'AWLs'
            ...
            if company and plan:
                ...
                if pd.notna(value):  # If the value is a number
                    structured_data[age]["companies"][company][plan]["additional_insurance_premium"] = float(value)
                else:  # If the value is not a number, keep it as a string
                    structured_data[age]["companies"][company][plan]["additional_insurance_premium"] = str(row[col])
